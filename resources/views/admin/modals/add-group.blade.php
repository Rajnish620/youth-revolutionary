<form method="POST" action="{{ route('admin.groups.store') }}" class="space-y-5" x-data="{
    selectedSeason: '',
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <select name="event_id" required
                ...
                <template x-for="ev in filteredEvents" :key="ev.id">
                    <option :value="ev.id" x-text="ev.title + ' (' + ev.category + ')'"></option>
                </template>
        ...
        <div>
            ...
            <input type="number" step="0.01" name="fee" placeholder="e.g. 100.00 or leave blank for free"
        ...
        <div>
            ...
            <input type="number" name="roll_sequence_start" value="1000" required placeholder="e.g. 1000"
                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-900 focus:bg-white focus:border-[#340C6F] outline-none transition-all">
