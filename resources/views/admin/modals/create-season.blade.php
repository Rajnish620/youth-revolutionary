<form method="POST" action="{{ route('admin.seasons.store') }}" class="space-y-5">
    @csrf

    <div>
        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Season Name *</label>
        <input type="text" name="name" required placeholder="e.g. Season 1"
    ...
    <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-4 mt-2">
        <button type="button" @click="$dispatch('close-modal', 'create-season')" 
